...
fn signal(data: [u8; FFT_SAMPLE_COUNT]) -> Vec<Complex<u8>> {
...
fn main() -> Result<(), io::Error> {
    ...
    loop {
        ...
        file.read(&mut buffer)?;
        let ffty: Vec<f64> = fft(signal(buffer), FFT_BIN_COUNT).iter().map(|val| val.norm()).collect();
        ...
        FFT_BIN_COUNT = calc_fft_bins(terminal.size().unwrap().width - 2f);
